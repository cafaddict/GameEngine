...
void VulkanRenderer_refac::Init() {
    ...
    m_FrameBuffer = std::make_shared<VulkanFrameBuffer>(
        m_Device, m_SwapChain, m_RenderPass, m_TransferCommandBuffer);
    ...
    m_Light = VulkanLight(glm::vec3(1.0f, 1.0f, 1.0f),
    ...
    m_CameraUniformBuffer = std::make_shared<VulkanBuffer_refac<VulkanCamera>>(
        m_Camera, offsets, m_Device, m_TransferCommandBuffer,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    m_LightUniformBuffer = std::make_shared<VulkanBuffer_refac<VulkanLight>>(
        m_Light, offsets, m_Device, m_TransferCommandBuffer,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    std::vector<glm::mat4> transformations;
    m_ModelStorageBuffer =
        std::make_shared<VulkanBuffer_refac<std::vector<glm::mat4>>>(
            transformations, offsets, m_Device, m_TransferCommandBuffer,
...
void VulkanRenderer_refac::Draw() {
    ...
    vkCmdSetViewport(m_CommandBuffer->getCommandBuffers()[m_CurrentFrame], 0, 1,
                     &viewport);

    VkRect2D scissor{};
    scissor.offset = {0, 0};
    scissor.extent = m_SwapChain->getSwapChainExtent();
    vkCmdSetScissor(m_CommandBuffer->getCommandBuffers()[m_CurrentFrame], 0, 1,
                    &scissor);
    ...
    if (!entities.empty()) {
        ...
        VkBuffer vertexBuffers[] = {
            std::get<VkBuffer>(m_VertexBuffer->getBuffer())};

        vkCmdBindVertexBuffers(
            m_CommandBuffer->getCommandBuffers()[m_CurrentFrame], 0, 1,
            vertexBuffers, offsets);
        vkCmdBindIndexBuffer(
            m_CommandBuffer->getCommandBuffers()[m_CurrentFrame],
            std::get<VkBuffer>(m_IndexBuffer->getBuffer()), 0,
            VK_INDEX_TYPE_UINT32);
    }
    for (const auto &entity : entities) {
        const auto graphicsPipeline = m_EntityPipelines[entity];
        auto descriptorSet = m_EntityDescriptorSets[entity];
        vkCmdBindPipeline(m_CommandBuffer->getCommandBuffers()[m_CurrentFrame],
                          VK_PIPELINE_BIND_POINT_GRAPHICS,
                          graphicsPipeline->getGraphicsPipeline());
        VkDeviceSize vertexOffset = m_VertexBuffer->getOffsets()[entity] *
                                    m_VertexBuffer->getDataSize();
        VkDeviceSize indexOffset =
            m_IndexBuffer->getOffsets()[entity] * m_IndexBuffer->getDataSize();
...
void VulkanRenderer_refac::createEntityResources() {
    ...
    for (auto &entity : entities) {
        ...
        for (int i = 0; i < MAX_FRAME_IN_FLIGHT; i++) {
            ...
            cameraBufferInfo.buffer = std::get<std::vector<VkBuffer>>(
                m_CameraUniformBuffer->getBuffer())[i];
            cameraBufferInfo.offset = 0;
            cameraBufferInfo.range = m_CameraUniformBuffer->getDataSize();

            VkDescriptorBufferInfo lightBufferInfo{};
            lightBufferInfo.buffer = std::get<std::vector<VkBuffer>>(
                m_LightUniformBuffer->getBuffer())[i];
            lightBufferInfo.offset = 0;
            lightBufferInfo.range = m_LightUniformBuffer->getDataSize();

            VkDescriptorBufferInfo modelBufferInfo{};
            modelBufferInfo.buffer = std::get<std::vector<VkBuffer>>(
                m_ModelStorageBuffer->getBuffer())[i];
        ...
        auto descriptorset = std::make_shared<VulkanDescriptorSet>(
            m_Device, m_EntityPipelines[entity], m_TransferCommandBuffer,
            texture_data, bufferInfos);

        m_EntityDescriptorSets[entity] = descriptorset;
    }

    m_VertexBuffer =
        std::make_shared<VulkanBuffer_refac<std::vector<VulkanVertex>>>(
            vertices, vertexOffsets, m_Device, m_TransferCommandBuffer,
            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    m_IndexBuffer = std::make_shared<VulkanBuffer_refac<std::vector<uint32_t>>>(
        indices, indexOffsets, m_Device, m_TransferCommandBuffer,
