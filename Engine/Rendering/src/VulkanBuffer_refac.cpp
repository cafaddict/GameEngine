...
namespace Engine {
void VulkanBuffer_refac::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
...
void VulkanBuffer_refac::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size,
...
uint32_t VulkanBuffer_refac::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties,
                                            std::shared_ptr<VulkanDevice> m_Device) {
...
VulkanVertexBuffer_refac::VulkanVertexBuffer_refac(std::vector<VulkanVertex> &vertices,
...
VulkanVertexBuffer_refac::~VulkanVertexBuffer_refac() {
...
VulkanIndexBuffer_refac::VulkanIndexBuffer_refac(std::vector<uint32_t> &indices, std::shared_ptr<VulkanDevice> device,
                                                 std::shared_ptr<VulkanCommandBuffer> transferCommandBuffer)
    : m_Device(device), m_TransferCommandBuffer(transferCommandBuffer) {
...
VulkanIndexBuffer_refac::~VulkanIndexBuffer_refac() {
    for (size_t i = 0; i < m_Buffers.size(); i++) {
        vkDestroyBuffer(m_Device->getLogicalDevice(), m_Buffers[i], nullptr);
        vkFreeMemory(m_Device->getLogicalDevice(), m_BuffersMemory[i], nullptr);
    }
