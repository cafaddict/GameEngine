...
void VulkanFrameBuffer::createFrameBuffer() {
    ...
    colorImageInfo.sharingMode = VK_SHARING_MODE_CONCURRENT;
    colorImageInfo.queueFamilyIndexCount = static_cast<uint32_t>(queueFamilyIndices.size());
    colorImageInfo.pQueueFamilyIndices = queueFamilyIndices.data();

    m_ColorImage = std::make_shared<VulkanImage>(m_Device, colorImageInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
    ...
    depthImageInfo.queueFamilyIndexCount = static_cast<uint32_t>(queueFamilyIndices.size());
    depthImageInfo.pQueueFamilyIndices = queueFamilyIndices.data();
    ...
    m_DepthImage = std::make_shared<VulkanImage>(m_Device, m_CommandBuffer, depthImageInfo,
                                                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_IMAGE_ASPECT_DEPTH_BIT);

    for (size_t i = 0; i < m_SwapChain->getSwapChainImageViews().size(); i++) {

        std::array<VkImageView, 3> attachments = {m_ColorImage->getImageView(), m_DepthImage->getImageView(),
