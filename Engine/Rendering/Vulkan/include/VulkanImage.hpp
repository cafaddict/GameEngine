...
class VulkanImage {
    ...
    VulkanImage(std::shared_ptr<VulkanDevice> device, std::shared_ptr<VulkanCommandBuffer> commandBuffer,
                VkImageCreateInfo imageInfo, VkMemoryPropertyFlags property, VkImageAspectFlags aspectFlags);
    ~VulkanImage();

    VkImageView getImageView() { return m_ImageView; }
    VkImage getImage() { return m_Image; }
    void transitionImageLayout(VkImageLayout oldLayout, VkImageLayout newLayout, VkFormat format, uint32_t mipLevels);
