...
template <typename T> class VulkanBuffer_refac {
    ...
    void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
        ...
        vkBeginCommandBuffer(m_TransferCommandBuffer->getCommandBuffers()[0], &beginInfo);

        VkBufferCopy copyRegion{};
        copyRegion.size = size;
        vkCmdCopyBuffer(m_TransferCommandBuffer->getCommandBuffers()[0], srcBuffer, dstBuffer, 1, &copyRegion);
        vkEndCommandBuffer(m_TransferCommandBuffer->getCommandBuffers()[0]);

        VkSubmitInfo submitInfo{};
        submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
        submitInfo.commandBufferCount = 1;
        submitInfo.pCommandBuffers = &m_TransferCommandBuffer->getCommandBuffers()[0];

        vkQueueSubmit(m_Device->getTransferQueue(), 1, &submitInfo, VK_NULL_HANDLE);
        vkQueueWaitIdle(m_Device->getTransferQueue());
        vkResetCommandBuffer(m_TransferCommandBuffer->getCommandBuffers()[0], 0);
    };
...
VulkanBuffer_refac<T>::VulkanBuffer_refac(T &data, std::unordered_map<std::shared_ptr<Entity>, size_t> entityOffsets,
    ...
    // if ((m_Usage == VK_BUFFER_USAGE_VERTEX_BUFFER_BIT || m_Usage == VK_BUFFER_USAGE_INDEX_BUFFER_BIT) && T_is_vecotr)
    // {
    if constexpr (is_vector<T>::value) {
